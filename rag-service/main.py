...
def ask_question(data: Question):
    ...
    if not passes_evidence_gate(question, docs, best_score, intent):
        ...
        with sessions_lock:
            session = sessions.get(session_id)
            if session:
                append_chat_exchange(session, question, INSUFFICIENT_CONTEXT_MESSAGE, [], mode)
                save_sessions_unlocked()
    ...
    if mode == "socratic":
        ...
        with sessions_lock:
            session = sessions.get(session_id)
            if session:
                append_chat_exchange(session, question, framed, citation_sources, mode)
                save_sessions_unlocked()
        return response_payload
    ...
    if grounded_answer == INSUFFICIENT_CONTEXT_MESSAGE:
        ...
        with sessions_lock:
            session = sessions.get(session_id)
            if session:
                append_chat_exchange(session, question, grounded_answer, citation_sources, mode)
                save_sessions_unlocked()
        return response_payload
    if grounded_answer:
        if ASK_REQUIRE_CITATIONS and not answer_contains_citation(grounded_answer, len(docs)):
            ...
            with sessions_lock:
                session = sessions.get(session_id)
                if session:
                    append_chat_exchange(session, question, INSUFFICIENT_CONTEXT_MESSAGE, citation_sources, mode)
                    save_sessions_unlocked()
            return response_payload
        logger.info(
...
def ask_question_stream(data: Question):
    ...
    if grounded_answer != INSUFFICIENT_CONTEXT_MESSAGE and grounded_answer:
        ...
        with sessions_lock:
            current_session = sessions.get(session_id)
            if current_session:
                append_chat_exchange(current_session, question, framed, citation_sources, mode)
                save_sessions_unlocked()
    ...
    def _generate_and_stream():
        try:
            ...
            with sessions_lock:
                current_session = sessions.get(session_id)
                if current_session:
                    append_chat_exchange(current_session, question, full_answer, citation_sources, mode)
                    save_sessions_unlocked()
        except Exception:
